...
- Inheritance
  ...
  - in dotnet interfaces play a major role, while in python a interface as such does not exist, they can be replicated with abstract base classes (ABC's) or with Protocols, but those are not the same as a dotnet interface, for instance a ABC can have implemented methods, while an interface cannot.
...
- Typing
  - While python is untyped, it is a best practice to type everything and we do, and we check those types using mypy or a equivalent.
- Private vs Public
...
1. **The goal of the python version of dotnet designs is not to complete coverage of all classes in the dotnet design.**
   - The goal is to maintain the same functionality and behavior, while adhering to python patterns.
   - This means that in some cases, things like config classes are not implemented, but keyword arguments are used instead, as they are more idiomatic to python, the same with options classes, and many static classes, which are not needed in python.
      - In some cases, it makes the internal logic easier to have a config or options class that is created based on the arguments, and that get's passed to other methods, this is fine and can mirror the dotnet design, but it is not needed in all cases. But in order to reduce the number of concepts that a developer has to interact with, we should not force the developer to use this internal class.
...
1. **Base classes and interfaces can be skipped, as long as SOLID is maintained.**
   - We should use Protocols for the outer public API's of classes that are often implemented for different services, such as ChatCompletionClients and VectorStores.
      - This can be accompanied by a Base Class (either ABC or not) that implements the Protocol, and could contain some shared functionality, but this is not needed in all cases.
      - The Protocol should have runtime_checking enabled and be used for as a field type as well, for instance the `services` field in the Kernel class, which is a dictionary the service name and the service instance, the type of this should be `dict[str, Protocol]`, this makes sure that outside developers can focus on implementing the Protocol and everything else just works.
      - The Protocols should all be located in a single `types.py` file to make it easy to consume.
